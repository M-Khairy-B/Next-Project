...
export default function Profile() {
    ...
    return (<> <div className='pt-24 w-[90%] m-auto'>
        <div className='flex flex-wrap gap-5 border-b-2 justify-between p-5 '>
            <div className='w-full lg:w-1/2 mx-auto'>
                <div className="w-full bg-white border border-gray-200 rounded-lg shadow">
                    <div className="flex flex-col items-center pb-10">
                        {isUpload ? <Alert sx={{ mt: '10px' }} icon={<CheckIcon fontSize="inherit" />} severity="success">
                            Upload Photo Successfull
                        </Alert> : null}
                        <img className="w-24 h-24 my-3 pt-2 rounded-full shadow-lg" src={userInfo?.photo} alt="Bonnie image" />
                        <h5 className="mb-1 text-xl font-medium">{userInfo?.name}</h5>
                        <div className='flex justify-between items-center w-full px-10 py-4'>
                            <span className="text-sm text-gray-500 dark:text-gray-400"><span className='text-base font-mono font-semibold'>E-mail: </span>{userInfo?.email}</span>
                            <span className="text-sm text-gray-500 dark:text-gray-400"><span className='text-base font-mono font-semibold'>Gender: </span>{userInfo?.gender}</span>
                        </div>
                        <div className='flex justify-between items-center w-full px-10 py-4'>
                            <span className="text-sm text-gray-500 dark:text-gray-400"><span className='text-base font-mono font-semibold'>DateOfBirth: </span> {userInfo?.dateOfBirth}</span>
             
                            <span className="text-sm text-gray-500 dark:text-gray-400"><span className='text-base font-mono font-semibold'>Age: </span>{age} years</span>
                        </div>
                        ...
                        <form onSubmit={handleProfilePhoto} className="flex items-center justify-center gap-5 mt-4 lg:mt-6 flex-col">
                                ...
                                <div className="flex items-center justify-center w-full">
                                    <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-10 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
                                        <div className="flex flex-col items-center w-full justify-center">
                                            <p className="text-sm text-gray-500 w-1/2 m-auto px-16 text-center"><span className="font-semibold"> upload</span> </p>
                                        </div>
                            ...
                            <Link href={'/changepassword'} className="flex flex-col items-center justify-center w-full h-10 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">Change Password</Link>
                        </form>
                    </div>
                </div>
            </div>
            <form onSubmit={handleSubmit} className='w-full lg:w-1/4 flex flex-col gap-5 mx-auto'>
                <h3 className='text-center text-3xl '>Add Post</h3>
                {isCreate ? <Alert icon={<CheckIcon fontSize="inherit" />} severity="success">
                    Added Post Successfull
                </Alert> : null}
                <textarea name='content' className='w-full resize-none h-40 border-2'></textarea>
                <div className="flex items-center justify-center w-full">
                    <label htmlFor="postPhoto" className="flex flex-col items-center justify-center w-full h-10 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
                        <div className="flex flex-col items-center justify-center">
                            <p className="text-sm text-gray-500 dark:text-gray-400"><span className="font-semibold">upload photo</span></p>
                        </div>
        ...
        <div className='flex flex-col items-center py-3'>
            <h3 className='text-center my-5 p-3 rounded-lg border-b-2 text-2xl font-mono'>Your Posts</h3>
            {userPosts?.map((post: any) => <Card key={post._id} sx={{ maxWidth: 545, my: '10px' }}>
                <div className='flex items-center justify-between'><CardHeader
                    ...
                    {userInfo?._id == post?.user?._id ? <button className='p-4' onClick={() => deletePosts(post?._id)}><BackspaceIcon/></button> : null}
                </div>
                <CardMedia
                    component="img"
                    height="194"

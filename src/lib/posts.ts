...
export const updatePost = createAsyncThunk('postsSlice/updatePost', async ({ allPosts, formData }) => {
console.log(  "updataposts postId ",  allPosts);
console.log(  "updataposts  formData",  formData);

try {
    const response = await axios.put(`https://linked-posts.routemisr.com/posts/${allPosts}`, formData, {
